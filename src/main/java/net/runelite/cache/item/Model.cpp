...
std::vector<int> Model::Model_sine;
std::vector<int> Model::Model_cosine;
std::vector<int> Model::field1890;

	Model::StaticConstructor::StaticConstructor()
	{
...
Model::StaticConstructor Model::staticConstructor;
	...
	void Model::calculateBoundsCylinder()
	{
		if (this->boundsType != 1)
		{
			...
			for (int var1 = 0; var1 < this->verticesCount; ++var1)
			{
				...
				if (-var3 > Renderable::modelHeight)
				{
					Renderable::modelHeight = -var3;
				}
			...
			this->XYZMag = static_cast<int>(std::sqrt(static_cast<double>(this->XYZMag)) + 0.99);
			this->radius = static_cast<int>(std::sqrt(static_cast<double>(this->XYZMag * this->XYZMag + Renderable::modelHeight * Renderable::modelHeight)) + 0.99);
			this->diameter = this->radius + static_cast<int>(std::sqrt(static_cast<double>(this->XYZMag * this->XYZMag + this->bottomY * this->bottomY)) + 0.99);
		}
	}

	void Model::rotateAndProject(std::shared_ptr<Graphics3D> graphics, int rotation_1, int yRotation, int zRotation, int xRotation, int xOffset, int yOffset, int zOffset)
	{
		field1871[0] = -1;
	...
	void Model::method0(std::shared_ptr<Graphics3D> graphics, bool var1, bool var3, int var4)
	{
			...
			else
			{
				...
				for (var26 = this->diameter - 1; var26 >= 0; --var26)
				{
					...
					if (var7 > 0)
					{
						...
						{
							var10 = var8[var9];
							signed char var31 = this->field1838[var10];
				...
				var10 = 0;
				var11 = field1872[10];
				std::vector<int> var28 = field1874[10];
				std::vector<int> var29 = field1857;
				if (var10 == var11)
				{
					var10 = 0;
					var11 = field1872[11];
				...
				for (var14 = 0; var14 < 10; ++var14)
				{
					...
					std::vector<int> var30 = field1874[var14];
	...
	void Model::method2706(std::shared_ptr<Graphics3D> graphics, int var1)
	{
		if (field1885[var1])
		{
		...
		else
		{
			...
			if (!this->field1841.empty() && this->field1841[var1] != -1)
			{
				int var5;
				int var6;
				int var7;
				if (!this->field1840.empty() && this->field1840[var1] != -1)
				{
			...
			{
				std::vector<int> field1889 = graphics->colorPalette;
	...
	void Model::method2707(std::shared_ptr<Graphics3D> graphics, int var1)
	{
