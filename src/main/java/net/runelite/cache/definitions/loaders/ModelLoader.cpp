...
namespace net::runelite::cache::definitions::loaders
{
	...
	std::shared_ptr<ModelDefinition> ModelLoader::load(int modelId, std::vector<signed char> &b)
	{
		std::shared_ptr<ModelDefinition> def = std::make_shared<ModelDefinition>();
		def->id = modelId;

		if (b[b.size() - 1] == -3 && b[b.size() - 2] == -1)
		{
			decodeType3(def, b);
		}
		else if (b[b.size() - 1] == -2 && b[b.size() - 2] == -1)
		{
			decodeType2(def, b);
		}
		else if (b[b.size() - 1] == -1 && b[b.size() - 2] == -1)
		{
			decodeType1(def, b);
		}
		else
	...
	void ModelLoader::decodeType3(std::shared_ptr<ModelDefinition> def, std::vector<signed char> &var1)
	{
		std::shared_ptr<InputStream> var2 = std::make_shared<InputStream>(var1);
		std::shared_ptr<InputStream> var3 = std::make_shared<InputStream>(var1);
		std::shared_ptr<InputStream> var4 = std::make_shared<InputStream>(var1);
		std::shared_ptr<InputStream> var5 = std::make_shared<InputStream>(var1);
		std::shared_ptr<InputStream> var6 = std::make_shared<InputStream>(var1);
		std::shared_ptr<InputStream> var7 = std::make_shared<InputStream>(var1);
		std::shared_ptr<InputStream> var8 = std::make_shared<InputStream>(var1);
		var2->setOffset(var1.size() - 26);
		...
		if (var11 > 0)
		{
			def->textureRenderTypes = std::vector<signed char>(var11);
			var2->setOffset(0);

			for (var28 = 0; var28 < var11; ++var28)
			{
				signed char var29 = def->textureRenderTypes[var28] = var2->readByte();
				if (var29 == 0)
				{
					++var25;
				}
		...
		def->vertexX = std::vector<int>(var9);
		def->vertexY = std::vector<int>(var9);
		def->vertexZ = std::vector<int>(var9);
		def->faceIndices1 = std::vector<int>(var10);
		def->faceIndices2 = std::vector<int>(var10);
		def->faceIndices3 = std::vector<int>(var10);
		if (var17 == 1)
		{
			def->packedVertexGroups = std::vector<int>(var9);
		}

		if (var12 == 1)
		{
			def->faceRenderTypes = std::vector<signed char>(var10);
		}

		if (var13 == 255)
		{
			def->faceRenderPriorities = std::vector<signed char>(var10);
		}
		else
		{
			def->priority = static_cast<signed char>(var13);
		}

		if (var14 == 1)
		{
			def->faceTransparencies = std::vector<signed char>(var10);
		}

		if (var15 == 1)
		{
			def->packedTransparencyVertexGroups = std::vector<int>(var10);
		}

		if (var16 == 1)
		{
			def->faceTextures = std::vector<short>(var10);
		}

		if (var16 == 1 && var11 > 0)
		{
			def->textureCoords = std::vector<signed char>(var10);
		}

		if (var18 == 1)
		{
			def->animayaGroups = std::vector<std::vector<int>>(var9);
			def->animayaScales = std::vector<std::vector<int>>(var9);
		}

		def->faceColors = std::vector<short>(var10);
		if (var11 > 0)
		{
			def->texIndices1 = std::vector<short>(var11);
			def->texIndices2 = std::vector<short>(var11);
			def->texIndices3 = std::vector<short>(var11);
		}
		...
		if (var18 == 1)
		{
			for (var51 = 0; var51 < var9; ++var51)
			{
				...
				def->animayaGroups[var51] = std::vector<int>(var52);
				def->animayaScales[var51] = std::vector<int>(var52);
		...
		{
			def->faceColors[var51] = static_cast<short>(var2->readUnsignedShort());
			if (var12 == 1)
			{
			...
			if (var16 == 1)
			{
				def->faceTextures[var51] = static_cast<short>(var7->readUnsignedShort() - 1);
			}

			if (!def->textureCoords.empty() && def->faceTextures[var51] != -1)
			{
				def->textureCoords[var51] = static_cast<signed char>(var8->readUnsignedByte() - 1);
			}
		}
		...
		for (var55 = 0; var55 < var11; ++var55)
		{
			...
			if (var56 == 0)
			{
				def->texIndices1[var55] = static_cast<short>(var2->readUnsignedShort());
				def->texIndices2[var55] = static_cast<short>(var2->readUnsignedShort());
				def->texIndices3[var55] = static_cast<short>(var2->readUnsignedShort());
			}
		}
	...
	void ModelLoader::decodeType2(std::shared_ptr<ModelDefinition> def, std::vector<signed char> &var1)
	{
		bool var2 = false;
		bool var3 = false;
		std::shared_ptr<InputStream> var4 = std::make_shared<InputStream>(var1);
		...
		if (var17 == 1)
		{
			for (var40 = 0; var40 < var9; ++var40)
			{
				...
				def->animayaGroups[var40] = std::vector<int>(var41);
				def->animayaScales[var40] = std::vector<int>(var41);
		...
		{
			def->faceColors[var40] = static_cast<short>(var4->readUnsignedShort());
			if (var12 == 1)
			{
				...
				if ((var41 & 2) == 2)
				{
					def->textureCoords[var40] = static_cast<signed char>(var41 >> 2);
		...
		for (var44 = 0; var44 < var11; ++var44)
		{
			...
			def->texIndices1[var44] = static_cast<short>(var4->readUnsignedShort());
			def->texIndices2[var44] = static_cast<short>(var4->readUnsignedShort());
			def->texIndices3[var44] = static_cast<short>(var4->readUnsignedShort());
		}

		if (!def->textureCoords.empty())
		{
			bool var47 = false;
			...
			for (var45 = 0; var45 < var10; ++var45)
			{
				...
				if (var46 != 255)
				{
					if (def->faceIndices1[var45] == (def->texIndices1[var46] & L'\uffff') && def->faceIndices2[var45] == (def->texIndices2[var46] & L'\uffff') && def->faceIndices3[var45] == (def->texIndices3[var46] & L'\uffff'))
					{
			...
			if (!var47)
			{
				def->textureCoords = std::vector<signed char>();
			}
		}

		if (!var3)
		{
			def->faceTextures = std::vector<short>();
		}

		if (!var2)
		{
			def->faceRenderTypes = std::vector<signed char>();
		}

	}

	void ModelLoader::decodeType1(std::shared_ptr<ModelDefinition> def, std::vector<signed char> &var1)
	{
		std::shared_ptr<InputStream> var2 = std::make_shared<InputStream>(var1);
		std::shared_ptr<InputStream> var3 = std::make_shared<InputStream>(var1);
		std::shared_ptr<InputStream> var4 = std::make_shared<InputStream>(var1);
		...
		if (var11 > 0)
		{
			...
			{
				signed char var27 = def->textureRenderTypes[var26] = var2->readByte();
				if (var27 == 0)
				{
					++var23;
				}
		...
		{
			def->faceColors[var49] = static_cast<short>(var2->readUnsignedShort());
			if (var12 == 1)
			{
			...
			if (var16 == 1)
			{
				def->faceTextures[var49] = static_cast<short>(var7->readUnsignedShort() - 1);
			}

			if (!def->textureCoords.empty() && def->faceTextures[var49] != -1)
			{
				def->textureCoords[var49] = static_cast<signed char>(var8->readUnsignedByte() - 1);
			}
		}
		...
		for (var53 = 0; var53 < var11; ++var53)
		{
			...
			if (var54 == 0)
			{
				def->texIndices1[var53] = static_cast<short>(var2->readUnsignedShort());
				def->texIndices2[var53] = static_cast<short>(var2->readUnsignedShort());
				def->texIndices3[var53] = static_cast<short>(var2->readUnsignedShort());
			}
		}
	...
	void ModelLoader::decodeOldFormat(std::shared_ptr<ModelDefinition> def, std::vector<signed char> &inputData)
	{
		bool usesFaceRenderTypes = false;
		bool usesFaceTextures = false;
		std::shared_ptr<InputStream> stream1 = std::make_shared<InputStream>(inputData);
		std::shared_ptr<InputStream> stream2 = std::make_shared<InputStream>(inputData);
		std::shared_ptr<InputStream> stream3 = std::make_shared<InputStream>(inputData);
		std::shared_ptr<InputStream> stream4 = std::make_shared<InputStream>(inputData);
		std::shared_ptr<InputStream> stream5 = std::make_shared<InputStream>(inputData);
		stream1->setOffset(inputData.size() - 18);
		...
		signed char offsetOfVertexFlags = 0;
		...
		def->vertexX = std::vector<int>(vertexCount);
		def->vertexY = std::vector<int>(vertexCount);
		def->vertexZ = std::vector<int>(vertexCount);
		def->faceIndices1 = std::vector<int>(faceCount);
		def->faceIndices2 = std::vector<int>(faceCount);
		def->faceIndices3 = std::vector<int>(faceCount);
		if (textureCount > 0)
		{
			def->textureRenderTypes = std::vector<signed char>(textureCount);
			def->texIndices1 = std::vector<short>(textureCount);
			def->texIndices2 = std::vector<short>(textureCount);
			def->texIndices3 = std::vector<short>(textureCount);
		}

		if (hasPackedVertexGroups == 1)
		{
			def->packedVertexGroups = std::vector<int>(vertexCount);
		}

		if (isTextured == 1)
		{
			def->faceRenderTypes = std::vector<signed char>(faceCount);
			def->textureCoords = std::vector<signed char>(faceCount);
			def->faceTextures = std::vector<short>(faceCount);
		}

		if (faceRenderPriority == 255)
		{
			def->faceRenderPriorities = std::vector<signed char>(faceCount);
		}
		else
		{
			def->priority = static_cast<signed char>(faceRenderPriority);
		}

		if (hasFaceTransparencies == 1)
		{
			def->faceTransparencies = std::vector<signed char>(faceCount);
		}

		if (hasPackedTransparencyVertexGroups == 1)
		{
			def->packedTransparencyVertexGroups = std::vector<int>(faceCount);
		}

		def->faceColors = std::vector<short>(faceCount);
		...
		{
			def->faceColors[i] = static_cast<short>(stream1->readUnsignedShort());
			if (isTextured == 1)
			{
				...
				{
					def->textureCoords[i] = static_cast<signed char>(faceTextureFlags >> 2);
		...
		for (int i = 0; i < textureCount; ++i)
		{
			...
			def->texIndices1[i] = static_cast<short>(stream1->readUnsignedShort());
			def->texIndices2[i] = static_cast<short>(stream1->readUnsignedShort());
			def->texIndices3[i] = static_cast<short>(stream1->readUnsignedShort());
		}
		...
		if (!def->textureCoords.empty())
		{
			...
			for (int i = 0; i < faceCount; ++i)
			{
				...
				if (coord != 255)
				{
					if (def->faceIndices1[i] == (def->texIndices1[coord] & L'\uffff') && def->faceIndices2[i] == (def->texIndices2[coord] & L'\uffff') && def->faceIndices3[i] == (def->texIndices3[coord] & L'\uffff'))
					{
						def->textureCoords[i] = -1;
					}
					else
