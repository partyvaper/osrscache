...
const std::shared_ptr<org::slf4j::Logger> InventoryDumper::logger = org::slf4j::LoggerFactory::getLogger(InventoryDumper::class);

//JAVA TO C++ CONVERTER TODO TASK: Most Java annotations will not have direct C++ equivalents:
//ORIGINAL LINE: @Test public void extract() throws java.io.IOException
	void InventoryDumper::extract()
	{
...
//ORIGINAL LINE: try (net.runelite.cache.fs.Store store = new net.runelite.cache.fs.Store(super))
		{
			...
			for (auto file : files->getFiles())
			{
				...
				Files::asCharSink(std::make_shared<File>(outDir, std::to_wstring(inv->id) + L".json"), Charset::defaultCharset()).write(gson->toJson(inv));
				++count;
			}
		}

		logger->info(L"Dumped {} inventories to {}", count, outDir);
	}
}
