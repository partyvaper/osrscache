...
namespace net::runelite::cache
{
	...
	using SpotAnimLoader = net::runelite::cache::definitions::loaders::SpotAnimLoader;
...
//ORIGINAL LINE: @Test @Ignore public void test() throws java.io.IOException
	void SpotanimDumperTest::test()
	{
		std::shared_ptr<File> dumpDir = folder->newFolder(L"spotanims");
		int count = 0;
...
//ORIGINAL LINE: try (net.runelite.cache.fs.Store store = new net.runelite.cache.fs.Store(StoreLocation.LOCATION))
		{
			...
			std::shared_ptr<Archive> archive = index->getArchive(ConfigType::SPOTANIM.getId());
			...
			std::shared_ptr<SpotAnimLoader> loader = std::make_shared<SpotAnimLoader>();

			for (auto file : files->getFiles())
			{
				std::vector<signed char> b = file->getContents();

				std::shared_ptr<SpotAnimDefinition> def = loader->load(file->getFileId(), b);
		...
		log::info(L"Dumped {} spotanims to {}", count, dumpDir);
	}
}
