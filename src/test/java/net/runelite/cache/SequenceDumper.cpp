...
namespace net::runelite::cache
{
	...
	using SequenceLoader = net::runelite::cache::definitions::loaders::SequenceLoader;
...
const std::shared_ptr<org::slf4j::Logger> SequenceDumper::logger = org::slf4j::LoggerFactory::getLogger(SequenceDumper::class);

//JAVA TO C++ CONVERTER TODO TASK: Most Java annotations will not have direct C++ equivalents:
//ORIGINAL LINE: @Test @Ignore public void extract() throws java.io.IOException
	void SequenceDumper::extract()
	{
...
//ORIGINAL LINE: try (net.runelite.cache.fs.Store store = new net.runelite.cache.fs.Store(super))
		{
			...
			std::shared_ptr<Archive> archive = index->getArchive(ConfigType::SEQUENCE.getId());
			...
			{
				std::shared_ptr<SequenceLoader> loader = std::make_shared<SequenceLoader>();
				std::shared_ptr<SequenceDefinition> seq = loader->load(file->getFileId(), file->getContents());

				Files::asCharSink(std::make_shared<File>(outDir, std::to_wstring(file->getFileId()) + L".json"), Charset::defaultCharset()).write(gson->toJson(seq));
				++count;
			}
		}

		logger->info(L"Dumped {} sequences to {}", count, outDir);
	}
}
