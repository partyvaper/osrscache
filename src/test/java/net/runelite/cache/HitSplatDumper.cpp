...
namespace net::runelite::cache
{
	...
	using HitSplatLoader = net::runelite::cache::definitions::loaders::HitSplatLoader;
...
//ORIGINAL LINE: try (net.runelite.cache.fs.Store store = new net.runelite.cache.fs.Store(StoreLocation.LOCATION))
		{
			...
			std::shared_ptr<Archive> archive = index->getArchive(ConfigType::HITSPLAT.getId());

			std::shared_ptr<HitSplatLoader> loader = std::make_shared<HitSplatLoader>();
			...
			for (auto file : files->getFiles())
			{
				...
				std::shared_ptr<HitSplatDefinition> def = loader->load(b);
		...
		log::info(L"Dumped {} hitsplats to {}", count, dumpDir);
	}
}
