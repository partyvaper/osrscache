...
namespace net::runelite::cache
{
	...
	using VarbitLoader = net::runelite::cache::definitions::loaders::VarbitLoader;
...
const std::shared_ptr<org::slf4j::Logger> VarbitDumper::logger = org::slf4j::LoggerFactory::getLogger(VarbitDumper::class);

//JAVA TO C++ CONVERTER TODO TASK: Most Java annotations will not have direct C++ equivalents:
//ORIGINAL LINE: @Test @Ignore public void extract() throws java.io.IOException
	void VarbitDumper::extract()
	{
...
//ORIGINAL LINE: try (net.runelite.cache.fs.Store store = new net.runelite.cache.fs.Store(super))
		{
			...
			std::shared_ptr<Archive> archive = index->getArchive(ConfigType::VARBIT.getId());
			...
			{
				std::shared_ptr<VarbitLoader> loader = std::make_shared<VarbitLoader>();
				std::shared_ptr<VarbitDefinition> varbit = loader->load(file->getFileId(), file->getContents());

				Files::asCharSink(std::make_shared<File>(outDir, std::to_wstring(file->getFileId()) + L".json"), Charset::defaultCharset()).write(gson->toJson(varbit));
				++count;
			}
		}

		logger->info(L"Dumped {} varbits to {}", count, outDir);
	}
}
